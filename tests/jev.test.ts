...
function harness(fetcher: typeof fetch = fakeFetch(), settings = config) {
  const sm = SessionManager.inMemory("/tmp/pi-jev-test");
...
test("scoring validates every probability before accepting a batch", async () => {
  ...
  await assert.rejects(score(messages, choices, config, undefined, (async () => new Response("not json")) as typeof fetch));
});
